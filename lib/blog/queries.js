...
import { isKnownCategory } from "@/lib/blog/categories";
import { blogCoverPath } from "@/lib/blog/urls";
import { normalizeRuns } from "@/lib/blog/rich-text";
...
/**
 * Posts gravados antes dos trechos com link tem `text` (e os itens de lista)
 * como string. Aqui eles ganham a forma canonica, para que o render tenha um
 * caminho unico. Esta e a segunda - e ultima - porta que tolera a forma antiga;
 * quando nao houver mais linha antiga no banco, esta funcao sai junto com a
 * tolerancia da ingestao. Ver `lib/blog/rich-text.js`.
 */
function migrateBlocks(blocks) {
  ...
  return blocks.map((block) => {
    ...
    if (typeof block.text === "string") {
      return { ...block, text: normalizeRuns(block.text) };
    }

    if (block.type === "list" && Array.isArray(block.items)) {
      return { ...block, items: block.items.map(normalizeRuns) };
    }
...
function toPost(row) {
  const post = {
    ...
    title: row.title,
    excerpt: row.excerpt,
    content: migrateBlocks(parseJson(row.content, [])),
    coverImage: {
...
/**
 * Uma pagina da listagem de uma categoria.
 *
 * NAO exclui o destaque, e a diferenca em relacao a `selectPage` e deliberada.
 * O indice exclui o destaque da grade porque ele ja esta no topo da MESMA
 * pagina; a listagem de categoria nao tem esse topo. Reusar a consulta do
 * indice aqui faria o post em destaque desaparecer da propria categoria dele -
 * justamente o post mais promovido, ausente de onde mais se procura.
 *
 * E essa unica clausula que faz as duas consultas serem duas.
 *
 * Categoria nao reconhecida devolve lista vazia, sem erro: a rota decide se
 * isso e 404 ou pagina vazia, e ela tem contexto para decidir.
 */
export async function selectCategoryPage(category, page) {
  const pageNumber = Number(page);
  ...
  const rows = await query(
    `SELECT ${CAMPOS}
     FROM dbo.posts p
     WHERE p.status = 'published' AND p.category = @categoria
     ORDER BY p.published_at DESC, p.slug
     OFFSET @pular ROWS FETCH NEXT @tamanho ROWS ONLY`,
    {
      categoria: { type: types.text, value: category },
      pular: { type: types.int, value: (pageNumber - 1) * POSTS_PER_PAGE },
      tamanho: { type: types.int, value: POSTS_PER_PAGE },
    }
  );
...
/**
 * Quantas paginas a categoria tem. Zero quando ela nao tem post publicado -
 * diferente do indice, que devolve no minimo 1 para sempre renderizar. Aqui o
 * zero e informacao: e como a rota sabe distinguir "categoria vazia" de
 * "pagina inexistente".
 */
export async function selectCategoryPageCount(category) {
  ...
  return Math.ceil((rows[0]?.total ?? 0) / POSTS_PER_PAGE);
}

/**
 * Contagem de publicados por categoria, em UMA consulta.
 *
 * Cinco consultas de contagem seriam cinco idas ao banco para um dado que muda
 * so quando um post entra. Categoria sem post publicado simplesmente nao
 * aparece no resultado - e assim que a barra sabe quais oferecer.
 *
 * @returns {Promise<Record<string, number>>} nome da categoria -> total
 */
export async function selectCategoryCounts() {
  const rows = await query(`
    SELECT p.category AS categoria, COUNT(*) AS total
    FROM dbo.posts p
    WHERE p.status = 'published'
    GROUP BY p.category`);
  ...
  for (const row of rows) {
    // Categoria fora das cinco pode existir no banco por escrita manual; ela
    // nao ganha secao, entao nao entra na contagem que alimenta a barra.
    if (isKnownCategory(row.categoria)) contagens[row.categoria] = row.total;
  }
